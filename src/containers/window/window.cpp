...
int main(int argc, char* argv[]) {
  ...
  auto window1 = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(window1), "window1");
  gtk_window_resize(GTK_WINDOW(window1), 640, 480);
  ...
  g_signal_connect (window1, "delete-event", G_CALLBACK(+[](GtkWidget* widget, GdkEvent* event, gpointer data)->bool {
    auto dialog = gtk_message_dialog_new(GTK_WINDOW(data), GTK_DIALOG_MODAL, GTK_MESSAGE_QUESTION, GTK_BUTTONS_YES_NO, "Are you sure you want exit? ?");
